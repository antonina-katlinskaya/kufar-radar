...
def save_kufar(db,items,profile_id):
    ...
    for x in items:
        seen.add(x.ad_id); f=fp([x.price_eur,x.price_byn,x.area,x.rooms,x.floor,x.address])
        old=cur.get(x.ad_id)
        if not old or old.get('fingerprint')!=f: changed.append(x)
        ...
        if not old or old.get('fingerprint')!=f:
    ...
    for aid,old in cur.items():
        if old.get('active') and aid not in seen: stm.append(('UPDATE kufar_ads SET active=0,last_seen_at=? WHERE ad_id=?',[ts,aid]))
...
def save_bir(db,items):
    ...
    for x in items:
        ...
        changed=(not old or any(old.get(k)!=v for k,v in [('price_regular_eur',x.price_regular_eur),('price_fast_eur',x.price_fast_eur),('area',x.area),('rooms',x.rooms),('floor',x.floor),('official_address',x.official_address)]))
    ...
    for key,old in cur.items():
        if old.get('active') and key not in seen: stm.append(('UPDATE bir_objects SET active=0,last_seen_at=? WHERE object_key=?',[ts,key]))
    for i in range(0,len(stm),75): db.batch(stm[i:i+75])
